...
async function main() {
  ...
  // v0.3.1: the `codex` child inherits this process's environment, so a `patrol` it runs from
  // its shell resolves the same credential file and authenticates as THIS seat rather than as
  // the operator. Best-effort — an unwritable credential dir must not stop the seat.
